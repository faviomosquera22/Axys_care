  ...
  return (
    <div className="stack">
      <div className="topbar">
        <div>
          <h1>Exámenes</h1>
          <p>Los exámenes viven dentro del encounter. Esta vista solo los consolida para revisión rápida.</p>
        </div>
      </div>

      <Card>
        <SectionHeading title="Filtrar por paciente" description="Elige un paciente y luego el encounter donde se solicitaron los exámenes." />
      ...
      <div className="workspace-grid">
        <Card>
          <SectionHeading title="Encuentros del paciente" description="Selecciona el episodio clínico a revisar." />
          {(encountersQuery.data ?? []).length ? (
            ...
            <div className="empty-state">
              ...
              <p>Solicita los exámenes desde Nueva atención o continúa uno existente.</p>
            </div>
          )}
        </Card>

        <Card className="workspace-aside">
          <SectionHeading title="Edición" description="Los exámenes se agregan dentro de la atención o la historia clínica." />
          {selectedEncounterId ? (
      ...
      {bundle ? (
        <Card>
          <SectionHeading title="Órdenes y resultados anexos" description="Solicitudes del encounter y archivos relacionados." />
          {(bundle.examOrders.length || bundle.attachments.length) ? (
              ...
              {bundle.examOrders.map((examOrder) => (
                <div key={examOrder.id} className="trace-row">
                  ...
                  <div className="btn-row">
                    ...
                    <StatusBadge label={examOrder.status} tone={examOrder.status === "pendiente" ? "warning" : "success"} />
                  </div>
                </div>
              ))}
            ...
            <div className="empty-state">
              <strong>Este encounter no tiene exámenes ni resultados anexos.</strong>
              <p>Solicítalos desde Nueva atención y aparecerán aquí y en la historia clínica.</p>
            </div>
          )}
        </Card>
      ) : null}
