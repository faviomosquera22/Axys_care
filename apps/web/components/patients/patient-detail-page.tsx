...
export function PatientDetailPage({ patientId }: { patientId: string }) {
  ...
  const patientQuery = useQuery({
    queryKey: ["patient", normalizedPatientId],
    queryFn: () => getPatient(client, normalizedPatientId),
    enabled: Boolean(normalizedPatientId),
  });
  const encountersQuery = useQuery({
    queryKey: ["encounters", normalizedPatientId],
    queryFn: () => listEncounters(client, normalizedPatientId),
    enabled: Boolean(normalizedPatientId),
  });
  ...
  const accessQuery = useQuery({
    queryKey: ["patient-access", normalizedPatientId],
    queryFn: () => listPatientAccess(client, normalizedPatientId),
    enabled: Boolean(normalizedPatientId),
  });

  usePatientRealtime(normalizedPatientId, [
    ["patient", normalizedPatientId],
    ["encounters", normalizedPatientId],
    ["patient-access", normalizedPatientId],
  ]);

  const patient = patientQuery.data;
  const isOwner = Boolean(user?.id && patient?.ownerUserId === user.id);
  const deleteMutation = useMutation({
    mutationFn: () => deletePatient(client, normalizedPatientId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["patients"] });
      queryClient.invalidateQueries({ queryKey: ["patient", normalizedPatientId] });
      queryClient.invalidateQueries({ queryKey: ["encounters", normalizedPatientId] });
      queryClient.invalidateQueries({ queryKey: ["patient-access", normalizedPatientId] });
